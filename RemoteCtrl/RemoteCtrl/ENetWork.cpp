...
{
	int ret = m_sock->send(buffer);	//TODO:Needs optimization. The sending was successful but incomplete.
	if (m_params.m_send)
...
{
	int ret = m_sock->sendto(buffer, addr);	//TODO:Needs optimization. The sending was successful but incomplete.
